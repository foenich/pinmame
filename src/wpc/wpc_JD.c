...
/* Column 1: Row 1 = LSB; Row 8 = MSB */
        tmpShift = 0;
        tmpShift |= ((fromNodes[3]>>3)&1)<<0; /* Row 1 */
        tmpShift |= ((fromNodes[0]>>2)&1)<<1; /* Row 2 */
        tmpShift |= ((fromNodes[3]>>4)&1)<<2; /* Row 3 */
        tmpShift |= ((fromNodes[5]>>3)&1)<<3; /* Row 4 */
        tmpShift |= ((fromNodes[3]>>0)&1)<<6; /* Row 7 */
        tmpShift |= (((fromNodes[3]>>7)&1) ||
          ((fromNodes[3]>>6)&1) ||
          ((fromNodes[3]>>5)&1)) <<7; /* Row 8 */
        coreGlobals.swMatrix[1] = tmpShift;

/* Column 2: Row 1 = LSB; Row 8 = MSB */
        tmpShift = 0;
        tmpShift |= ((fromNodes[4]>>2)&1)<<4; /* Row 5 */
        tmpShift |= ((fromNodes[0]>>5)&1)<<5; /* Row 6 */
        tmpShift |= ((fromNodes[4]>>4)&1)<<6; /* Row 7 */
        coreGlobals.swMatrix[2] = tmpShift;

/* Column 3 */
        tmpShift = 0;
        tmpShift |= ((fromNodes[4]>>1)&1)<<2; /* Row 3 */
        tmpShift |= ((fromNodes[1]>>0)&1)<<3; /* Row 4 */
        tmpShift |= (((fromNodes[5]>>7)&1) ||
          ((fromNodes[5]>>5)&1))<<4; /* Row 5 */
        tmpShift |= ((fromNodes[4]>>3)&1)<<5; /* Row 6 */
        coreGlobals.swMatrix[3] = tmpShift;

/* Column 4 */
        tmpShift = 0;
        tmpShift |= ((fromNodes[1]>>2)&1)<<0; /* Row 1 */
        tmpShift |= ((fromNodes[1]>>4)&1)<<1; /* Row 2 */
        tmpShift |= ((fromNodes[1]>>0)&1)<<2; /* Row 3 */
        coreGlobals.swMatrix[4] = tmpShift;

/* Column 5 */
        tmpShift = 0;
        tmpShift |= (((fromNodes[2]>>5)&1) ||
          ((fromNodes[3]>>1)&1)) <<0; /* Row 1 */
        tmpShift |= (((fromNodes[1]>>7)&1) ||
          ((fromNodes[2]>>7)&1)) <<1; /* Row 2 */
        tmpShift |= ((fromNodes[0]>>4)&1)<<2; /* Row 3 */
        tmpShift |= ((fromNodes[4]>>7)&1)<<3; /* Row 4 */
        tmpShift |= ((fromNodes[4]>>7)&1)<<4; /* Row 5 */
        tmpShift |= ((fromNodes[4]>>6)&1)<<5; /* Row 6 */
        tmpShift |= ((fromNodes[4]>>5)&1)<<6; /* Row 7 */
        tmpShift |= ((fromNodes[4]>>5)&1)<<7; /* Row 8 */
        coreGlobals.swMatrix[5] = tmpShift;

/* Column 6 */
        tmpShift = 0;
        tmpShift |= ((fromNodes[5]>>2)&1)<<3; /* Row 4 */
        tmpShift |= ((fromNodes[4]>>0)&1)<<5; /* Row 6 */
        tmpShift |= ((fromNodes[0]>>3)&1)<<7; /* Row 8 */
        coreGlobals.swMatrix[6] = tmpShift;

/* Column 7 */
        tmpShift = 0;
        tmpShift |= ((fromNodes[0]>>7)&1)<<1; /* Row 2 */
        tmpShift |= ((fromNodes[2]>>2)&1)<<2; /* Row 3 */
        tmpShift |= ((fromNodes[2]>>3)&1)<<3; /* Row 4 */
        tmpShift |= ((fromNodes[5]>>6)&1)<<4; /* Row 5 */
        tmpShift |= (((fromNodes[2]>>1)&1) ||
          ((fromNodes[2]>>0)&1)) <<5; /* Row 6 */
        coreGlobals.swMatrix[7] = tmpShift;

/* Column 8 */
        tmpShift = 0;
        tmpShift |= ((fromNodes[1]>>3)&1)<<0; /* Row 1 */
...
/* Node 0 */
	toNode[0] = 0x08; /* node 0, 8 data bytes */

	/* Solenoids nr. in manual - 1 */
	tmpShift = 0;
	tmpShift = 1<<7; /* Enable right flipper; tmpShift |= ((coreGlobals.solenoids>>4)&1); */
	tmpShift |= 0<<6; /* magnet */
	tmpShift |= 1<<5; /* Enable outhole kicker */
	tmpShift |= ((coreGlobals.solenoids>>12)&1)<<4; /* shooterlane feeder */
	tmpShift |= 1<<3; /* Enable right Slingshot tmpShift |= ((coreGlobals.solenoids>>15)&1)<<3;*/
	tmpShift |= 0<<2; /* lock diverter */
	tmpShift |= 0<<1; /* lock kickback */
	tmpShift |= ((coreGlobals.solenoids>>4)&1)<<0; /* reset drop targets */
	toNode[1] = tmpShift;
...
/*	toNode[5] = tmpShift; */
	...
	tmpShift = 0;
	tmpShift |= 0;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<6;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<5;
	tmpShift |= ((coreGlobals.lampMatrix[3]>>1)&1)<<4;
	tmpShift |= ((coreGlobals.lampMatrix[3]>>2)&1)<<3;
	tmpShift |= ((coreGlobals.lampMatrix[3]>>3)&1)<<2;
	tmpShift |= 0;
	tmpShift |= ((coreGlobals.lampMatrix[5]>>6)&1);
	toNode[2] = tmpShift;

	/* Lamp[col-1]>>(row-1)  */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.lampMatrix[5]>>1)&1)<<7;
	tmpShift |= ((coreGlobals.lampMatrix[2]>>4)&1)<<6;
	tmpShift |= ((coreGlobals.lampMatrix[6]>>1)&1)<<5;
	tmpShift |= ((coreGlobals.lampMatrix[6]>>2)&1)<<4;
	tmpShift |= ((coreGlobals.lampMatrix[6]>>3)&1)<<3;
	tmpShift |= ((coreGlobals.lampMatrix[2]>>5)&1)<<2;
	...
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<6;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<5;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<4;
	tmpShift |= 0;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<2;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<1;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1);
	toNode[4] = tmpShift;
	...
	/* Lamp[col-1]>>(row-1) */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<7;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<6;
	tmpShift |= ((coreGlobals.lampMatrix[0]>>4)&1)<<5;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<4;
	tmpShift |= ((coreGlobals.lampMatrix[5]>>4)&1)<<3;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<2;
	tmpShift |= ((coreGlobals.lampMatrix[0]>>4)&1)<<1;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1);
	toNode[5] = tmpShift;
	...
	tmpShift = 0;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<7;
	tmpShift |= ((coreGlobals.lampMatrix[5]>>2)&1)<<6;
	tmpShift |= ((coreGlobals.lampMatrix[5]>>7)&1)<<5;
	tmpShift |= ((coreGlobals.lampMatrix[5]>>0)&1)<<4;
	...
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<5;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<4;
	tmpShift |= ((coreGlobals.solenoids>>21)&1)<<3; // flasher 22
	tmpShift |= ((coreGlobals.solenoids>>21)&1)<<2; // flasher 22
	tmpShift |= ((coreGlobals.solenoids>>21)&1)<<1; // flasher 22

	tmpShift |= ((coreGlobals.lampMatrix[0]>>5)&1); // Dummy for Prep 2 red
...
/* Node 1 */
	toNode[0] = 0x19; /* node 1, 9 data bytes */

	/* Solenoids:  nr. in manual - 1 */
	tmpShift = 0;
	tmpShift = 1<<7; /* Enable left flipper */
	tmpShift |= ((coreGlobals.solenoids>>8)&1)<<6; /* kick back */
	tmpShift |= 1<<5; /* enable lift sling */
	tmpShift |= ((coreGlobals.solenoids>>1)&1)<<4; /* upper kick back */
	tmpShift |= ((coreGlobals.solenoids>>2)&1)<<3; /* ball popper VUK */
...
/*	toNode[5] = tmpShift; */
	...
	tmpShift = 0;
	tmpShift |= 1<<7;
	tmpShift |= (((coreGlobals.lampMatrix[6]>>5)&1) ||
	  ((coreGlobals.lampMatrix[6]>>6)&1) ||
	  ((coreGlobals.lampMatrix[6]>>7)&1)) <<6;
	tmpShift |= ((coreGlobals.swMatrix[5]>>5)&1)<<5; /* lit if drop target 'A' is down */
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<4;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<3;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<2;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<1;
	tmpShift |= ((coreGlobals.lampMatrix[4]>>2)&1)<<0;
	toNode[2] = tmpShift;

	/* Lamp[col-1]>>(row-1)  */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.lampMatrix[4]>>6)&1)<<7;
	tmpShift |= ((coreGlobals.solenoids>>19)&1)<<6;
	tmpShift |= ((coreGlobals.solenoids>>17)&1)<<5;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>2)&1)<<4;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>3)&1)<<3;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>2)&1)<<2;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>2)&1)<<1;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1);
	toNode[3] = tmpShift;
	...
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<7;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<6;
	tmpShift |= ((coreGlobals.solenoids>>16)&1)<<5;
	tmpShift |= ((coreGlobals.solenoids>>22)&1)<<4;
	tmpShift |= ((coreGlobals.solenoids>>18)&1)<<3;
	tmpShift |= ((coreGlobals.lampMatrix[4]>>3)&1)<<2;
	tmpShift |= ((coreGlobals.lampMatrix[4]>>1)&1)<<1;
	tmpShift |= ((coreGlobals.lampMatrix[4]>>0)&1)<<0;
	toNode[4] = tmpShift;
	...
	tmpShift |= ((coreGlobals.lampMatrix[3]>>7)&1)<<3;
	tmpShift |= ((coreGlobals.lampMatrix[3]>>7)&1)<<2;
	tmpShift |= ((coreGlobals.lampMatrix[4]>>4)&1)<<1;
	tmpShift |= ((coreGlobals.lampMatrix[1]>>0)&1)<<0;
	toNode[5] = tmpShift;

	/* Lamp[col-1]>>(row-1) */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.lampMatrix[0]>>0)&1)<<7;
	tmpShift |= ((coreGlobals.lampMatrix[4]>>5)&1)<<6;
	tmpShift |= ((coreGlobals.lampMatrix[3]>>6)&1)<<5;
	tmpShift |= ((coreGlobals.lampMatrix[3]>>5)&1)<<4;
	tmpShift |= ((coreGlobals.lampMatrix[3]>>4)&1)<<3;
	tmpShift |= ((coreGlobals.lampMatrix[2]>>5)&1)<<2;
	tmpShift |= ((coreGlobals.lampMatrix[2]>>5)&1)<<1;
	tmpShift |= ((coreGlobals.lampMatrix[2]>>5)&1)<<0;
	toNode[6] = tmpShift;

	/* Lamp[col-1]>>(row-1) */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>5)&1)<<7;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<6;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>5)&1)<<5;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>4)&1)<<4;
	tmpShift |= ((coreGlobals.solenoids>>21)&1)<<3;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<2;
	tmpShift |= ((coreGlobals.solenoids2>>7)&1)<<1;
	tmpShift |= ((coreGlobals.lampMatrix[7]>>0)&1)<<0;
	toNode[7] = tmpShift;

	/* Lamp[col-1]>>(row-1) */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.lampMatrix[1]>>4)&1)<<7;
	tmpShift |= ((coreGlobals.lampMatrix[2]>>1)&1)<<6;
	tmpShift |= ((coreGlobals.lampMatrix[0]>>1)&1)<<5; // Dummy for Prep 1 red
	tmpShift |= ((coreGlobals.lampMatrix[0]>>2)&1)<<4; // Dummy for Prep 1 yellow
	tmpShift |= ((coreGlobals.lampMatrix[0]>>3)&1)<<3; // Dummy for Prep 1 green
	tmpShift |= ((coreGlobals.lampMatrix[2]>>1)&1)<<2; // Dummy for Prep 3 red
	tmpShift |= ((coreGlobals.lampMatrix[2]>>2)&1)<<1; // Dummy for Prep 3 yellow
	tmpShift |= ((coreGlobals.lampMatrix[2]>>3)&1)<<0; // Dummy for Prep 3 green
	toNode[8] = tmpShift;

	/* Lamp[col-1]>>(row-1) */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.lampMatrix[1]>>1)&1)<<7; // Dummy for Prep 4 red
	tmpShift |= ((coreGlobals.lampMatrix[1]>>2)&1)<<6; // Dummy for Prep 4 yellow
	tmpShift |= ((coreGlobals.lampMatrix[1]>>3)&1)<<5; // Dummy for Prep 4 green
	tmpShift |= ((coreGlobals.lampMatrix[1]>>5)&1)<<4; // Dummy for Prep 5 red
	tmpShift |= ((coreGlobals.lampMatrix[1]>>6)&1)<<3; // Dummy for Prep 5 yellow
	tmpShift |= ((coreGlobals.lampMatrix[1]>>7)&1)<<2; // Dummy for Prep 5 green
	toNode[9] = tmpShift;
	
	write(serbus, toNode, 10); /* 9 bytes + addr&length */
...
/* Node 2 */
	...
	tmpShift |= 0<<6; /* ramp diverter */
	tmpShift |= ((coreGlobals.solenoids>>25)&1)<<5; /* Flasher 7 */
	tmpShift |= ((coreGlobals.solenoids>>24)&1)<<4; /* Flasher 6 */
	tmpShift |= ((coreGlobals.solenoids>>23)&1)<<3; /* Flasher 5 */
	tmpShift |= ((coreGlobals.solenoids>>22)&1)<<2; /* Flasher 4 */
	tmpShift |= ((coreGlobals.solenoids>>21)&1)<<1; /* Flasher 3 */
	tmpShift |= ((coreGlobals.solenoids>>20)&1)<<0; /* Flasher 2 */
	toNode[1] = tmpShift;
	...
	/* Lamp[col-1]>>(row-1)  */
	tmpShift = 0;
	tmpShift |= ((coreGlobals.solenoids>>20)&1)<<7;
	tmpShift |= ((coreGlobals.solenoids>>21)&1)<<6;
	tmpShift |= ((coreGlobals.solenoids>>22)&1)<<5;
	tmpShift |= ((coreGlobals.solenoids>>23)&1)<<4;
	tmpShift |= ((coreGlobals.solenoids>>24)&1)<<3;
	tmpShift |= ((coreGlobals.solenoids>>25)&1)<<2;
	tmpShift |= ((coreGlobals.solenoids>>26)&1)<<1;
	tmpShift |= ((coreGlobals.solenoids>>27)&1)<<0;
	toNode[2] = tmpShift;
